...
class SimpleCheckout extends Simple {
    ...
    public function loadSimpleSession() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'GET') {
            ...
            foreach ($addressFields as $field) {
                if (!empty($this->session->data['payment_address'][$field])) { //  && empty($this->session->data['simple']['payment_address'][$field])) {
                    $this->session->data['simple']['payment_address'][$field] = $this->session->data['payment_address'][$field];
                }
            }

            foreach ($addressFields as $field) {
                if (!empty($this->session->data['shipping_address'][$field])) { //  && empty($this->session->data['simple']['shipping_address'][$field])) {
                    $this->session->data['simple']['shipping_address'][$field] = $this->session->data['shipping_address'][$field];
                }
            }
