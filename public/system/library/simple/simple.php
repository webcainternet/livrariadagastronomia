...
class Simple {
    ...
    public function addLinksToStaticFiles() {
        ...
        $disableJQueryUI = $this->getSettingValueDirectly('', '', 'disableJQueryUI');
        ...
        if (isset($this->session->data['user_id']) && !$minify) {
    ...
    public function updateFields() {
        ...
        foreach ($round as $fieldSettings) {
            ...
            if ($value === '' && !empty($fieldSettings['autoreload']) && $this->_fields[$block][$fieldSettings['id']]['object'] == 'address') {
    ...
    public function getRowsAsHtml() {
        ...
        foreach ($this->_displayedRows[$block] as $row) {
            if ($row['rowType'] == 'header') {
                ...
                $templateData['page'] = $this->_page;
                $templateData['site_key'] = '';

                if ($this->config->get('config_google_captcha_status') && $this->config->get('config_google_captcha_public')) {
                    $templateData['site_key'] = $this->config->get('config_google_captcha_public');
                }

                if ($this->config->get('config_captcha') == 'google_captcha' && $this->config->get('google_captcha_status') && $this->config->get('google_captcha_key')) {
                    $templateData['site_key'] = $this->config->get('google_captcha_key');
                }
